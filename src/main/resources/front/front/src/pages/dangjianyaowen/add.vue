...
<div :style='{"border":"1px solid #9ce3b5","padding":"20px","boxShadow":"0px 2px 6px #ccc,inset 0px 0px 600px 0px #e0f8e8","margin":"20px auto 0","borderRadius":"16px","background":"#fff","width":"1200px","position":"relative"}'>
    <el-form
      ...
      label-width="80px"
    >
          <el-form-item :style='{"border":"0px solid #dfdfdf","padding":"10px","boxShadow":"0px 0px 0px #eee","margin":"0 0 8px 0","borderRadius":"8px","background":"none"}' label="党建标题" prop="dangjianbiaoti">
            <el-input v-model="ruleForm.dangjianbiaoti" 
                placeholder="党建标题" clearable ></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"border":"0px solid #dfdfdf","padding":"10px","boxShadow":"0px 0px 0px #eee","margin":"0 0 8px 0","borderRadius":"8px","background":"none"}' label="视频" prop="shipin">
            <file-upload
            tip="点击上传视频"
            action="file/upload"
            :limit="1"
          ...
          <el-form-item :style='{"border":"0px solid #dfdfdf","padding":"10px","boxShadow":"0px 0px 0px #eee","margin":"0 0 8px 0","borderRadius":"8px","background":"none"}' label="简介" prop="jianjie">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="简介"
          ...
          <el-form-item :style='{"border":"0px solid #dfdfdf","padding":"10px","boxShadow":"0px 0px 0px #eee","margin":"0 0 8px 0","borderRadius":"8px","background":"none"}' label="详情内容" prop="xiangqingneirong">
            <editor 
  ...
  export default {
    data() {
      return {
        ...
        rules: {
          dangjianbiaoti: [
            { required: true, message: '党建标题不能为空', trigger: 'blur' },
          ],
          fengmian: [
          ],
          jianjie: [
    ...
    methods: {
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            if(o=='dangjianbiaoti'){
              this.ruleForm.dangjianbiaoti = obj[o];
              this.ro.dangjianbiaoti = true;
              continue;
            }
            if(o=='fengmian'){
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('dangjianyaowen/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('dangjianyaowen/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('dangjianyaowen/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
